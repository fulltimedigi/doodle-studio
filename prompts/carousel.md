...
- Two extra slide roles are available when the topic earns them, at most one of each in a carousel:
  - `"role": "stat"` — one number that carries the whole slide. Needs `stat` (the number alone, Arabic-Indic digits, ≤ 5 characters), `unit` (what it counts, ≤ 6 words), `body` (why it matters, ≤ 20 words) and `source` (where the number came from, ≤ 14 words). **Never invent a number.** Use it only for a figure the brief or the brand guide actually states, and put the real source in `source`. If you have no sourced number, do not use this role.
  - `"role": "chat"` — a short exchange showing a real customer moment. Needs `messages`: 2–4 items of `{ "who": "them" | "us" | "miss", "text": "..." }` where `them` is the customer, `us` is the brand, and `miss` marks what was missing or went wrong (shown in red). Each text ≤ 12 words. `body` is the one-line takeaway.
