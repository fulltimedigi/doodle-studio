// Raster line-art -> ordered centerline pen strokes, pure JavaScript (no Python needed).
// ink mask (Jimp) -> skeleton polylines (skeleton-tracing-js, MIT) -> merge touching chains ->
// simplify -> group into visual objects -> human-like drawing order -> SVG (strokes + original colors).
import Jimp from 'jimp';
import TraceSkeleton from 'skeleton-tracing-js/trace_skeleton.vanilla.js';

const dist = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1]);

function inkMask(img, threshold) {
  const w = img.bitmap.width, h = img.bitmap.height, d = img.bitmap.data;
  const gray = new Float32Array(w * h);
  for (let i = 0; i < w * h; i++) {
    const a = d[i * 4 + 3] / 255;
    const g = (0.299 * d[i * 4] + 0.587 * d[i * 4 + 1] + 0.114 * d[i * 4 + 2]) * a + 255 * (1 - a);
    gray[i] = g;
  }
  // local adaptive threshold (box mean, radius r) AND global threshold — works for scans and photos alike
...
// median half-width of the ink along the skeleton -> pen width in image px
function strokeWidth(mask, w, h, lines) {
  const samples = [];
  for (const l of lines) for (let i = 0; i < l.length; i += 3) {
    const [x, y] = l[i].map(Math.round); let rad = 0;
...
export async function imageToStrokes(file, { threshold = 150, color = '#1a1a1a', fill = 'original', maxSize = 1400, minLen = 12, simplifyEps = 1.2 } = {}) {
  let img = await Jimp.read(file);
  if (Math.max(img.bitmap.width, img.bitmap.height) > maxSize) img.scaleToFit(maxSize, maxSize);
  const { mask, w, h } = inkMask(img, threshold);
  const { polylines } = TraceSkeleton.fromBoolArray(Array.from(mask), w, h);
  ...
  if (fill === 'original') {
    ...
    fillLayer = `<image class="fill-src" href="data:image/png;base64,${png.toString('base64')}" x="0" y="0" width="${w}" height="${h}"/>`;
  }
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" data-stroke-width="${width.toFixed(2)}">${fillLayer}<g class="strokes" fill="none" stroke="${color}" stroke-width="${width.toFixed(2)}" stroke-linecap="round" stroke-linejoin="round">${paths}</g></svg>`;
  return { svg, strokes: lines.length, total_length: Math.round(lines.reduce((a, l) => a + arcLen(l), 0)), stroke_width: width, size: [w, h] };
}
