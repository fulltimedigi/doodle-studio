...
await page.route('https://generativelanguage.googleapis.com/**', async (route) => {
  ...
  if (u.includes('predictLongRunning')) { const b = JSON.parse(route.request().postData()); const inst = b.instances[0]; calls.push({ model: u.match(/models\/([^:]+)/)[1], img: !!inst.image, refs: !!inst.referenceImages, pg: b.parameters.personGeneration, nov: 'numberOfVideos' in b.parameters, neg: !!b.parameters.negativePrompt, dur: b.parameters.durationSeconds, res: b.parameters.resolution });
...
await page.fill('#brief', 'x'); if (mode === 'standard') await page.selectOption('#tier', 'standard'); if (mode === 'lite') await page.selectOption('#tier', 'lite'); await page.click('#go'); await page.waitForFunction(() => !document.querySelector('#out')?.hidden && document.querySelectorAll('#clipsEd .clip').length > 0, null, { timeout: 30000 });

page.on('dialog', (d) => d.accept());
await page.click('#shootBtn');
await page.waitForFunction(() => /كل اللقطات جاهزة|فشلت/.test(document.querySelector('#msg2').textContent), null, { timeout: 120000 });
