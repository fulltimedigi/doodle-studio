...
```bash
npm test        # كل شيء: الصفحات في متصفح حقيقي + المحرّكات
npm run test:fast   # بلا متصفح — ثانيتان
```

`test/` يفتح الصفحات الإحدى عشرة فعليًا ويفشل عند أي خطأ في الـ console أو أي ملف ناقص،
ويركّب ريلًا قصيرًا ويطبع PDF ويصدّر شرائح الكاروسيل بالمقاسات الثلاثة ثم يفحص الناتج نفسه.
بلا متصفح مثبّت تُتخطّى اختبارات المتصفح بوضوح بدل أن تفشل. يعمل نفسه على كل دفعة عبر
`.github/workflows/test.yml`.

`src/site-map.mjs` هو الوصف الوحيد لتخطيط الملفات المنشورة: منه ينسخ البناء الساكن
(`scripts/build-site.mjs`) ومنه يقدّم الخادم (`src/server.mjs`)، فلا يختلف المساران.
...
|---|---|---|
| صفحة الهبوط | `landing.html` | صفحة واحدة تجمع البريد/الواتساب وتسلّم الكتيّب فورًا — تُصدَّر **ملف HTML واحد** يحمل التصميم والنموذج والكتيّب نفسه، ترفعه على Netlify Drop أو استضافتك. بيانات الزوار تذهب إلى Formspree أو Apps Script أو أي webhook، وفيها زر يختبر الرابط قبل النشر |
| لوحة المحتوى | `board.html` | كل ما ستنشره في مكان واحد: بند لكل يوم ومنصة، حالته (فكرة ← كتابة ← جاهزة ← مجدولة ← نُشرت)، والمادة المصنوعة مربوطة به. تستورد خطة الشهر، وتُصدّر CSV وتقويم ICS ونسخة احتياطية. كل شيء على جهازك |
| خطة الشهر | `plan.html` | محاور الشهر وبنوده موزّعة على أيامه ومناسباته المحسوبة — وكل بند يفتح أداته بالموضوع جاهزًا |
| هوية البراند | `index.html` | الاسم، اللهجة، النبرة، الألوان، الشعار — تُطبَّق على كل الوحدات. تُملأ بالكتابة، أو **برفع دليل الهوية PDF وصور الهوية البصرية** فتُقرأ منها الألوان وأسلوب الصور |
...
| كاروسيل | `carousel.html` | ٧ شرائح بأربعة قوالب — تصدير بثلاثة مقاسات (1:1 و4:5 و9:16) في ZIP مرتّب |
...
- المصدر: `docs/brand/fulltimedigi-brief.md` (الدليل الكامل) و`assets/brand/fulltimedigi.json` (النسخة التي يحمّلها زر «هوية FullTimeDigi الجاهزة»).
- «هوية البراند» صارت تحمل حقلين إضافيين يُحقنان في كل أداة: **دليل الهوية** (التموضع، وجهة النظر، الرسائل الأساسية، الشخصية، محاور المحتوى) و**أسلوب الصور** (ماذا نعرض وماذا نتجنب — يُلحق بكل برومبت صورة).
- لهجة جديدة «عربية بيضاء عصرية» لجمهور الخليج ومصر معًا، ونبرتان جديدتان «صادق ومباشر» و«تعليمي».
- الكاروسيل يدعم شرائح Typography فقط (`visual: ""`) وصورة سفلية على خلفية بيضاء في قالب البراند؛ الإعلان يتحوّل تلقائيًا لاتجاهات «مشكلة / رؤية / وجهة نظر» عندما يقول الدليل إن المنتج غير مطلق بعد.
